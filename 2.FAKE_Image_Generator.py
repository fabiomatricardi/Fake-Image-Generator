...
def perform_search(query: str, progress=gr.Progress()) -> Tuple[List, str, str]:
    """Main search function with diffusion animation"""
    if not query.strip():
        empty_gallery = [None] * TOP_K
        ...
        return empty_gallery, empty_details, empty_animation
    
    try:
        progress(0, desc="Loading FAISS index...")
        ...
        for i in range(TOP_K):
            if i < len(image_paths) and image_paths[i] and os.path.exists(image_paths[i]):
                try:
                    ...
                    gallery_label = f"Score: {scores[i]:.4f}"
                    ...
                    <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); 
                        ...
                        <div style="background: rgba(255,255,255,0.95); padding: 15px; 
                                    border-radius: 5px; font-size: 16px; line-height: 1.6; 
                                    color: #333;">
                            <strong style="color: #667eea; font-size: 18px;">📝 Original Prompt (Chinese):</strong><br>
                            {prompt_display}
                        </div>
        ...
        detailed_html_str = "<div style='font-family: Arial, sans-serif;'>" + "".join(detailed_html) + "</div>"
        
        # Generate diffusion animation HTML
        animation_html = generate_animation_html(image_paths, prompts, scores)
        
        progress(1.0, desc="Done!")
        return gallery_results, detailed_html_str, animation_html
        ...
        return empty_gallery, error_msg, empty_animation
    except Exception as e:
...
def create_interface():
    ...
    with gr.Blocks(
        ...
        gr.Markdown(
            ...
            **Model:** bge-m3-Q8_0-GGUF (Multilingual) | **Database:** kaupane/nano-banana-pro-gen | **Results:** Top 4 matches
            """
        )
        ...
        # Results Section - Static Gallery
        gr.Markdown("### 📸 Static Results")
        
        with gr.Row():
            results_gallery = gr.Gallery(
                label="Top 4 Matches (Static)",
                show_label=True,
                columns=2,
                rows=2,
                object_fit="contain",
        ...
        search_btn.click(
            fn=perform_search,
            inputs=[query_input],
            outputs=[results_gallery, detailed_results_html, diffusion_animation]
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"   ./server -m models/bge-m3-Q8_0.gguf --port 8080 --embedding")
        exit(1)
    except Exception as e:
